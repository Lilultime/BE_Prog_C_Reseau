...
[ok] la gestion des options -s, -u,  le nombre et Taille par défaut des messages , nom de la machine destinataire,  numéro de port du puit est opérationnelle

[ok] la création du socket UDP local et la construction de l'adresse du socket distant est correcte

[ok] l’envoi de  messages en utilisant l'appel système sendto (qui retourne une valeur >0 correspondant à la taille de chaque message envoyé) est fonctionnelle 

[ok] la réception côté puit (sur nc ou la version "enseignant" de tsock) des messages envoyés par votre client est correcte

### Avancement Point sur la partie puit UDP.

[ok] la prise en compte de l' option -p est correcte

[ok] la création du socket UDP local du puit et la construction de son adresse est correcte

[ok] la réception et l'affichage des messages reçus sont corrects

[ok] l'échange de messages tq décrits dans le cahier des charges entre vos versions source et puit de la version 1 de "tsock" est opérationnel
...
[ok] la création du socket TCP local du puit et la construction de son adresse est correcte

[ok] l'acceptation de la demande d'établissement de connexion de la primitive "accept" est effective 

[ok] la réception et l'affichage des messages reçus  (en utilisant nc ou la version "enseignant" de tsock comme source)  sont corrects 

[ok] l'échange de messages entre vos versions source et puit de la version 1 de "tsock" est opérationnel
...
[ok] Le formattage et affichage des messages selon le cahier des charges sont opérationnels

[ok] la prise en compte de l'option -n en émission aussi bien avec UDP et TCP est correcte

[ok] la prise en compte de l'option -n en réception avec UDP est correcte

[ok] la prise en compte de l'option -n en réception avec TCP est correcte

[ok] la prise en compte de l'option -l est correcte
...
[ok] demande d'établissement de connexion TCP avec le serveur BAL fonctionnelle

[ok] transmission du PDU applicatif en charge de le déclarer en tant que émetteur et éventuellement indiquer la taille et nombre de messages qu'il émet est fonctionnelle

[ok] émission des messages au serveur BAL fonctionnelle

[ok] cloture de la connexion TCP avec le serveur BAL fonctionnelle
...
[ok] Acceptation des demandes de connexions TCP et l'identification,par le traitement du PDU applicatif, du rôle "Emetteur" de l'application qui a initiée la demande de connexion et ses caractéristiques  (nombre et taille des messages)  est fonctionnelle

[ok] recherche de la boîte aux lettres (BAL) relative à un récepteur fonctionnelle

[ok] creation/rajout de BAL d'un récepteur fonctionnelle

[ok] rajout d'un message dans une BAL d'un récepteur à la dernière position est fonctionnelle


### Pour les fonctionnalités du serveur BAL (option "-b") relatives aux interactions avec les Récepteurs
 veuillez indiquer les étapes que vous avez validées

[ok] acceptation des demandes de connexions TCP et l'identification, par le traitement du PDU applicatif, du rôle "Récepteur" de l'application qui a initiée la demande de connexion est fonctionnelle

[ok] vérification de la présence d'une BAL associée à un récepteur et renvoi de l'accès à la liste des messages disponibles pour le récepteur est fonctionnelle


### Pour les fonctionnalités du "Récepteur" (l'option "-e" de tsock) 
veuillez indiquer les étapes que vous avez validées.

[ok] demande de l'établissement de connexion TCP avec le serveur BAL est fonctionnelle

[ok] Transmission du PDU applicatif en charge de le déclarer en tant que récepteur  est fonctionnelle

[ok] prise en charge la réception et affichage de ses éventuels messages transmis par le serveur BAL jusqu'à la fermeture de la connexion TCP déclenchée par le serveur BAL est fonctionnelle

[ok] parcours des messages d'une BAL opérationnel
